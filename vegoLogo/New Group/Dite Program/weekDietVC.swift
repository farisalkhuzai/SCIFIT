...
import UIKit
import Firebase
class weekDietVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    var diet:Diet?
    var month:Int?
...
//        Daylabel.text = "يوم " + (diet?.day)!
    ...
    func loadData(){
        ...
        ref.child("users").child(userID!).child("\(month)").observe(.value, with: { (snapshot) in
            
            guard let value = snapshot.value as? [String: Any] else { return }
            do {
                // Get user value
                let jsonData = try JSONSerialization.data(withJSONObject: value)
                self.diet = try? JSONDecoder().decode(Diet.self, from: jsonData)
...
//                self.loadView.stopAnimating()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = weeksTV.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! weekDietCell
//        cell.weekLabel.text = diet?.month[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if weeknumber == 1 {
            self.selectedmonth = (diet?.week1)!
            
        }
        if weeknumber == 2 {
            self.selectedmonth = (diet?.week2)!
            
        }
        if weeknumber == 3 {
            self.selectedmonth = (diet?.week3)!
            
        }
        if weeknumber == 4 {
            self.selectedmonth = (diet?.week4)!
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "segueweek"  {
            if let destinationVC = segue.destination as? dietVC {
                destinationVC.diet = self.weeks
